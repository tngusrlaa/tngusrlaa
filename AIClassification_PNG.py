...
# 데이터 전처리
file_path = "C:/Users/KIM/AppData/Local/JetBrains/PyCharm2024.1/cpython-cache/Users/KIM/PycharmProjects/250409_jpgstructralanly/250429/png.csv"
...
cols_to_drop = ['AI_service_label', 'Subfolder Name', 'File Name']
X = df.drop(columns=cols_to_drop)
y = df['AI_service_label']

# 최종 학습에 사용될 피처 리스트 출력
print(f"최종 학습 피처 개수: {len(X.columns)}")
print(f"최종 학습 피처 목록:\n{X.columns.tolist()}")

# 타겟 레이블 정보 출력
print(f"\n🎯 타겟 레이블 명: y (AI_service_label)")
print(f"클래스 분포:\n{y.value_counts().sort_index()}")
print(f"클래스 라벨 목록: {sorted(y.unique())}")


# 결측치 처리 함수
def handle_missing_data(X):
    # 결측치인 경우 'Missing'으로 처리
    X = X.apply(lambda col: col.fillna('Missing'))
    return X
